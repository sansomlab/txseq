...
db="/well/sansom/users/odq545/work/txseq/hsc_example/featurecounts/csvdb"
sample_table="/well/sansom/users/odq545/work/txseq/hsc_example/samples.tsv"
...
plot_color="condition"    #e.g. "timepoint"
plot_shape="tissue"    #e.g. "knockdown"
plot_label="cell_type"    #e.g."replicate"

#
exclude <- c()
experiment_groups <- c("tissue","cell_type","condition")
plot_groups <- c("tissue","cell_type","condition")
nreplicates=2
```
...
```{r exp-tsne, fig.cap='UMAP projection of samples based genes with significantly variable expression between the experimental groups', fig.height=4, fig.width=6,  echo=FALSE, message=FALSE}

umap_frame <- as.data.frame(umap_out$layout)
rownames(umap_frame) <- colnames(filteredVSTMat)
colnames(umap_frame) <- c("UMAP1", "UMAP2")

at <- merge(umap_frame, columnData, by=0)
